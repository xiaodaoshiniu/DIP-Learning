...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        points = torch.as_tensor(points3D_xyz, dtype=torch.float32)
        dists = torch.cdist(points, points)
        dists = dists.topk(K + 1, largest=False).values[:, 1:]
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Paper Eq. (6): Sigma = R S S^T R^T.
        # `scales` are standard deviations along the local Gaussian axes.
        L = torch.bmm(R, S)
