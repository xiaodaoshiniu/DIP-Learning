...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        det = torch.linalg.det(covs2D).clamp_min(eps)
        ...
        gaussian = norm * torch.exp(exponent.clamp(min=-80.0, max=20.0)) ## (N, H, W)
    ...
    def forward(
        ...
        # 7. Compute weights
        # T_i = prod_{j<i}(1-alpha_j), weights_i = T_i alpha_i.
        transmittance = torch.cumprod(
            torch.cat([torch.ones_like(alphas[:1]), 1.0 - alphas + 1e-10], dim=0),
            dim=0,
        )[:-1]
