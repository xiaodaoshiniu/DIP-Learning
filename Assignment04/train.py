...
def main():
    ...
    final_epoch = start_epoch + config.num_epochs
    trainer.save_checkpoint(final_epoch)
    print(f"Saved final checkpoint: {os.path.join(args.checkpoint_dir, f'checkpoint_{final_epoch:06d}.pt')}")
    print(f"Training wall time: {elapsed:.2f} seconds")
    if device.type == 'cuda':
